 ...
 * This component renders the primary High-Level Command Center for administrators. 
 * It features real-time traffic visualization (Recharts), system health telemetry 
 * (latency, database status, uptime, memory), and quick-access tactical operation 
 * modules for managing the portfolio orchestration layers.
 */

import React, { useState, useEffect } from 'react';
// Material UI components for the dashboard layout and notification systems
import { Box, Typography, Container, Grid, Paper, Stack, Button, Snackbar, Alert } from '@mui/material';
// Framer Motion for subtle interactive transitions
import { motion } from 'framer-motion';
// Icons for navigation and status indicators
import { LayoutDashboard, Users, FolderKanban, Cpu, LogOut, Settings, ExternalLink, Activity } from 'lucide-react';
// Custom context and hooks for authentication and live telemetry data
import { useAdmin } from '../context/AdminContext';
import { useNavigate } from 'react-router-dom';
import useLiveAnalytics from '../hooks/useLiveAnalytics';
// Recharts components for rendering the traffic stream visualization
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';
import SEO from '../components/SEO';
// API services for system integrity checks and operational locks
...
const AdminDashboard = () => {
    ...
    // State for tracking the core vital signs of the backend infrastructure
     ...
     * Ensures the user is authenticated. Redirects to login if the session is invalid.
     */
    useEffect(() => {
        const checkAuth = () => {
        ...
        // Brief delay to allow the auth context to hydrate from localStorage
     ...
     * Periodically probes the system integrity endpoint to update the health monitor.
     */
    useEffect(() => {
     ...
     * Activates or deactivates the global system lock (Maintenance Mode).
     */
    ...
    /**
     * [handleLogout]
     * Terminates the current administrative session.
     */
    const handleLogout = () => {
        logoutAdmin();
    ...
    /**
     * [openModule]
     * Routes the administrator to specific management sub-modules.
     */
    const openModule = (moduleName) => {
        const moduleRoutes = {
            ...
            'OPERATIONAL SECURITY': '/admin/management?tab=7'
        };
        ...
        setToast({ open: true, message: `${moduleName} module is syncing. Please try again.` });
    };

    // Configuration for the "Tactical Operations" module grid
    const modules = [
    ...
    return (
        <Box sx={{ 
            minHeight: '100vh', bgcolor: '#04070a', pt: 12, pb: 10,
            ...
            <Container maxWidth="xl">
                ...
                <Box sx={{ 
                    ...
                    <Box>
                        <Stack direction="row" spacing={3} alignItems="center" sx={{ mb: 2 }}>
                            <Box sx={{ 
                                ...
                                border: '1px solid rgba(51, 204, 255, 0.3)', borderRadius: '16px', color: '#33ccff'
                            }}>
                                <LayoutDashboard size={28} />
                            </Box>
                            <Box>
                                <Typography variant="h3" sx={{ fontWeight: 900, fontFamily: 'Syncopate', color: 'white', letterSpacing: -2 }}>SYSTEM_CORE</Typography>
                                <Typography variant="overline" sx={{ color: '#33ccff', fontWeight: 900, letterSpacing: 5 }}>ADMINISTRATIVE_COMMAND_CENTER</Typography>
                            </Box>
                        </Stack>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, pl: 1, color: '#444', fontFamily: 'monospace', fontSize: '0.75rem' }}>
                    ...
                    <Button startIcon={<LogOut size={18} />} onClick={handleLogout}
                        sx={{ 
                            color: '#ff3366', fontWeight: 900, fontFamily: 'Syncopate', fontSize: '0.75rem',
                            border: '1px solid rgba(255, 51, 102, 0.3)', px: 4, py: 1.5, borderRadius: '12px'
                        }}
                    >
                        TERMINATE_SESSION
                    </Button>
                ...
                <Grid container spacing={4}>
                    {/* Diagnostic Monitor - Large Left Panel (Traffic Stream) */}
                    <Grid item xs={12} lg={8}>
                        <Paper sx={{ 
                            p: 6, bgcolor: 'rgba(10, 15, 25, 0.6)', backdropFilter: 'blur(20px)',
                            border: '1px solid rgba(255,255,255,0.05)', borderRadius: '40px', height: '100%'
                        }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 6 }}>
                                ...
                                <Box sx={{ px: 3, py: 1, bgcolor: 'rgba(0, 255, 204, 0.05)', border: '1px solid rgba(0, 255, 204, 0.1)', borderRadius: '50px', color: '#00ffcc', display: 'flex', alignItems: 'center', gap: 2 }}>
                                    <Activity size={18} />
                                    <Typography sx={{ fontWeight: 900, fontFamily: 'monospace' }}>{activeSessions} NODES_ACTIVE</Typography>
                                </Box>
                            </Stack>

                            {/* [Recharts Area Chart] Renders the 7-day traffic history */}
                                ...
                                {history && history.length > 0 ? (
                                    <ResponsiveContainer width="100%" height={350}>
                                        <AreaChart data={history}>
                                            ...
                                            <XAxis dataKey="date" stroke="#444" fontSize={11} fontWeight={900} axisLine={false} tickLine={false} tickFormatter={(str) => str.split('-').slice(1).join('/')} />
                                            <YAxis hide />
                                            <Tooltip contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid rgba(51, 204, 255, 0.3)', borderRadius: '12px', fontSize: '0.8rem', fontFamily: 'Syncopate', backdropFilter: 'blur(10px)' }} itemStyle={{ color: '#33ccff', fontWeight: 900 }} />
                                            <Area type="monotone" dataKey="count" stroke="#33ccff" strokeWidth={4} fillOpacity={1} fill="url(#colorCount)" animationDuration={3000} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                ) : (
                    ...
                    {/* Right Panel: Vital Signs Monitor */}
                    <Grid item xs={12} lg={4}>
                        <Paper sx={{ p: 6, bgcolor: 'rgba(4, 7, 10, 0.4)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '40px', height: '100%', backdropFilter: 'blur(15px)' }}>
                                ...
                                {[
                                    ...
                                    <Box key={i}>
                                        ...
                                        <Box sx={{ height: 4, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 2, overflow: 'hidden' }}>
                                            <motion.div initial={{ width: 0 }} animate={{ width: `${stat.p}%` }} transition={{ duration: 1.5, ease: 'circOut' }}
                            ...
                            {/* System Lock Toggle (Maintenance Mode) */}
                            <Box sx={{ mt: 8, pt: 6, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                <Button fullWidth variant="outlined" onClick={handleToggleMaintenance} disabled={updatingMaintenance}
                                    sx={{ 
                                        py: 2.5, borderRadius: '16px', fontFamily: 'Syncopate', fontWeight: 900, fontSize: '0.75rem',
                    ...
                    <Grid container spacing={4}>
                        {modules.map((m, i) => (
                            <Grid item xs={12} sm={6} lg={3} key={i}>
                                <Paper onClick={() => openModule(m.name)}
                                    sx={{ 
                                        p: 5, bgcolor: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.03)', borderRadius: '24px', 
                                        transition: '0.4s', cursor: 'pointer', position: 'relative', overflow: 'hidden',
                                        '&:hover': { transform: 'translateY(-12px)', borderColor: m.color, background: 'rgba(255,255,255,0.02)' }
                                    }}>
                                    <Box sx={{ mb: 4, color: m.color, p: 2, bgcolor: `${m.color}11`, width: 'max-content', borderRadius: '12px' }}>{m.icon}</Box>
                ...
                {/* ── [SESSION EVENT FEED] ── */}
                <Box sx={{ mt: 10, p: 6, bgcolor: 'rgba(0,0,0,0.8)', border: '1px solid rgba(255,255,255,0.02)', borderRadius: '30px', fontFamily: 'monospace' }}>
            ...
            <Snackbar open={toast.open} autoHideDuration={4000} onClose={() => setToast({ open: false, message: '' })} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}>
                <Alert onClose={() => setToast({ open: false, message: '' })} severity="success" variant="filled" sx={{ bgcolor: '#00ffcc', color: '#000', fontWeight: 900, fontFamily: 'Syncopate', fontSize: '0.7rem' }}>
                    {toast.message}
                </Alert>
            </Snackbar>
