  ...
  return (
    <IconButton
      ...
      sx={{
        ...
        bgcolor: 'rgba(1, 4, 9, 0.8)', border: '1px solid rgba(255, 255, 255, 0.1)',
        backdropFilter: 'blur(10px)', color: copied ? '#00ffcc' : '#888',
...
const Documentation = ({ profile }) => {
  ...
  useEffect(() => {
    ...
    const fetchDocs = async () => {
      try {
        ...
        setReadmeContent(readmeText.trim().startsWith('<!DOCTYPE html>') ? '# SYSTEM_INITIALIZING...' : readmeText);
        setProjectContent(projectText.trim().startsWith('<!DOCTYPE html>') ? '# ARCHITECTURE_SYNC_PENDING...' : projectText);
      } catch (error) {
    ...
    const tree = [
      { name: 'client', type: 'folder', children: [
        { name: 'src', type: 'folder', children: [
          { name: 'components', type: 'folder', children: [{ name: 'LanguageSelector.js', type: 'file', highlight: true }] },
          { name: 'pages', type: 'folder', children: [{ name: 'Documentation.js', type: 'file', highlight: true }] }
        ]}
      ]},
      { name: 'server', type: 'folder', children: [{ name: 'data.json', type: 'file', highlight: true }] }
    ];
        ...
        {items.map((item, i) => (
          <Box key={i} sx={{ mb: 1 }}>
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box sx={{ color: item.type === 'folder' ? '#33ccff' : '#64748b' }}>{item.type === 'folder' ? <Layers size={14} /> : <Hash size={14} />}</Box>
              <Typography sx={{ color: item.highlight ? 'white' : '#cbd5e1', fontSize: '0.85rem', fontFamily: 'monospace' }}>{item.name}</Typography>
            </Stack>
  ...
  const markdownStyles = {
    color: '#cbd5e1', lineHeight: 2, fontSize: '1rem',
    '& h1': { fontSize: '2.2rem', borderBottom: '2px solid rgba(51, 204, 255, 0.2)', pb: 3, color: '#33ccff' },
    '& h2': { fontSize: '1.6rem', borderLeft: '4px solid #ff3366', pl: 3, py: 1 },
    '& code': { bgcolor: 'rgba(0, 255, 204, 0.1)', color: '#00ffcc', p: '2px 8px', borderRadius: 1.5, fontFamily: 'monospace' },
    '& pre': { bgcolor: '#05070a', p: 4, borderRadius: 4, border: '1px solid rgba(255,255,255,0.08)', overflowX: 'auto' }
  };
  ...
  return (
    <Box sx={{ bgcolor: '#02040a', minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <AnimatePresence>
        {isSearching && (
          <Box sx={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsSearching(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)' }} />
             <motion.div initial={{ scale: 0.95, opacity: 0, y: -20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: -20 }}
                style={{ position: 'relative', width: '100%', maxWidth: 500, background: '#0d1117', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' }}>
                <Box sx={{ p: 2, borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 2 }}>
                   <Search size={20} color="#33ccff" />
                   <input autoFocus placeholder="FIND_DOCUMENTATION..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                      style={{ background: 'none', border: 'none', color: 'white', flexGrow: 1, outline: 'none', fontSize: '1.2rem', fontFamily: 'Syncopate', fontWeight: 900 }} />
                </Box>
                <Box sx={{ maxHeight: 400, overflowY: 'auto', p: 1.5 }}>
                   {filteredNav.map((item, i) => (
                      <Box key={i} onClick={() => handleNav(item)} sx={{ p: 2, borderRadius: 2, display: 'flex', alignItems: 'center', gap: 2, cursor: 'pointer', '&:hover': { bgcolor: 'rgba(255,255,255,0.03)' } }}>
      ...
      {/* ── [TOP_BAR NAVIGATION] ── */}
      <Box sx={{ height: 80, borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', px: { xs: 2, md: 5 }, bgcolor: 'rgba(1, 4, 9, 0.9)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 1000, justifyContent: 'space-between' }}>
        <Stack direction="row" spacing={3} alignItems="center">
          <Box sx={{ p: 1, bgcolor: '#ff3366', borderRadius: '10px', color: 'white' }}><Terminal size={22} /></Box>
          <Typography sx={{ color: 'white', fontWeight: 900, letterSpacing: 2, fontFamily: 'Syncopate' }}>SYSTEM_DOCS</Typography>
        </Stack>
        <Box onClick={() => setIsSearching(true)} sx={{ width: 300, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', p: 1.5, px: 2, borderRadius: 2, color: '#555', display: { xs: 'none', md: 'flex' }, justifyContent: 'space-between', cursor: 'pointer' }}>
           <Typography variant="caption" sx={{ fontWeight: 800 }}>QUICK_FIND...</Typography>
      ...
      <Box sx={{ flexGrow: 1, display: 'flex' }}>
         {!isMobile && (
           <Box sx={{ width: 320, borderRight: '1px solid rgba(255,255,255,0.05)', p: 4, bgcolor: 'rgba(1, 4, 9, 0.7)' }}>
             <Typography variant="overline" sx={{ color: '#333', fontWeight: 900, mb: 4, display: 'block' }}>RESOURCES</Typography>
             <Stack spacing={1.5}>
                {navItems.slice(0, 5).map((item) => (
                  ...
                  >
                    {item.icon} <Typography sx={{ fontSize: '0.75rem', fontWeight: 900 }}>{item.name.toUpperCase()}</Typography>
                  </Box>
                ))}
             </Stack>
           </Box>
           ...
           <AnimatePresence mode="wait">
              {isLoading ? (
                <Box sx={{ height: '50vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Typography sx={{ mt: 4, color: '#334155', fontFamily: 'monospace' }}>ESTABLISHING_TLS_CONNECTION...</Typography>
                </Box>
              ) : (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                   <Typography variant="h2" sx={{ color: 'white', fontWeight: 900, mb: 6 }}>{activeSection.toUpperCase()}</Typography>
                   <Box sx={{ p: { xs: 3, md: 8 }, bgcolor: 'rgba(1, 4, 9, 0.4)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.03)' }}>
                      <ReactMarkdown components={{
                          ...
                          pre: ({ children }) => (
                             <Box sx={{ position: 'relative', my: 4, '&:hover .copy-button': { opacity: 1 } }}>
                                <CopyButton text={children?.props?.children || ''} />
                                <Box component="pre" sx={markdownStyles['& pre']}>{children}</Box>
                             </Box>
                          )
                      }}>
                        {activeSection === 'readme' ? readmeContent : (activeSection === 'explanation' ? translatedDoc : '# Section PENDING')}
                      </ReactMarkdown>
                   </Box>
                </motion.div>
              )}
