    ...
    >
      ...
      */}
      <Box className="circuit-bg" sx={{
        ...
        backgroundSize: '15px 15px', transition: '0.5s opacity'
      }} />
      ...
      <Stack spacing={3} sx={{ position: 'relative', zIndex: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ p: 1.5, borderRadius: 3, bgcolor: `${color}15`, color: color, display: 'flex', boxShadow: `0 0 20px ${color}22` }}>
            <Icon size={32} />
          </Box>
          <Typography variant="overline" sx={{ color: '#64748b', fontWeight: 900, letterSpacing: 2, fontFamily: 'Syncopate', fontSize: '0.55rem' }}>LVL_EXPERT</Typography>
        </Box>

        {/* Category Title and animated line indicator */}
        <Box>
          <Typography variant="h3" sx={{
            fontSize: '1.2rem', fontWeight: 800, fontFamily: 'Outfit', letterSpacing: -0.5, color: 'white', mb: 1
          }}>
            {title}
          </Typography>
          <Box className="expertise-bar" sx={{ height: 3, width: '30%', bgcolor: color, transition: '0.8s width cubic-bezier(0.23, 1, 0.32, 1)', borderRadius: 4, boxShadow: `0 0 15px ${color}` }} />
        </Box>
          ...
          {skills.map((skill, idx) => {
            ...
            return (
              <Tooltip key={skill} title={`Mastery Level: ${mastery}% - Production Validated`} arrow placement="top">
                <Box
                  sx={{
                    px: 1.8, py: 0.8, borderRadius: '10px', bgcolor: 'rgba(255,255,255,0.02)',
                    ...
                    '&:hover': {
                      ...
                      boxShadow: `0 0 20px ${color}33`, transform: 'translateY(-3px)'
                    }
                  }}
                >
                  <Typography sx={{ fontSize: '0.8rem', fontWeight: 700 }}>{skill}</Typography>
                  <Typography sx={{ color: color, fontSize: '0.65rem', fontWeight: 900, opacity: 0.7 }}>{mastery}%</Typography>
                </Box>
              </Tooltip>
            );
          })}
      ...
      <Box sx={{
        ...
        borderRadius: '50%', bgcolor: color, animation: 'pulse-neon 2.2s infinite'
      }} />
    </Paper>
  </motion.div>
));
    ...
    <Box id="skills" sx={{ py: 15, position: 'relative' }}>
      ...
      <Box sx={{ mb: 12, textAlign: 'center' }}>
        <Stack direction="row" justifyContent="center" alignItems="center" spacing={2} sx={{ mb: 3 }}>
            <Box sx={{ p: 1.5, bgcolor: 'rgba(51, 204, 255, 0.1)', borderRadius: 2.5, color: '#33ccff', boxShadow: '0 0 20px rgba(51, 204, 255, 0.1)' }}>
                <Activity size={28} />
            </Box>
            <Typography variant="overline" sx={{ color: '#00ffcc', fontWeight: 900, letterSpacing: 8, fontFamily: 'Syncopate', fontSize: '0.75rem' }}>
        ...
        <Typography
          variant="h2"
          sx={{
            fontFamily: 'Syncopate', fontWeight: 900, letterSpacing: -3, mb: 2,
            fontSize: { xs: '2.5rem', md: '5.5rem' }, color: 'white',
            textShadow: '0 0 50px rgba(255,255,255,0.05)'
          }}
        >
          CORE <span style={{ color: '#6366f1', textShadow: '0 0 30px rgba(99, 102, 241, 0.4)' }}>ARSENAL</span>
        </Typography>
        <Typography variant="body1" sx={{ color: '#94a3b8', maxWidth: 800, mx: 'auto', fontSize: '1.25rem', lineHeight: 1.8, fontWeight: 400 }}>
