...
exports.protect = asyncHandler(async (req, res, next) => {
    ...
    if (!token || token === 'null' || token === 'undefined') {
        return res.status(401).json({ success: false, error: 'Access denied. Valid administrative token required.' });
    }
    ...
    try {
        ...
        if (isDbConnected && !isPortableUser) {
            ...
            if (!req.admin) {
                return res.status(401).json({ success: false, error: 'Unauthorized access. Session terminated.' });
            }
        } else {
